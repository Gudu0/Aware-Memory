...
public final class TileWorld {
    ...
    public void addEarned(float amount) {
    ...
    private void refreshSplitterVariantAt(int cx, int cy) {
        ...
        if (!(te instanceof org.gudu0.AwareMemory.entities.SplitterEntity s)) return;
        ...
        if (lOk && rOk) s.setVariant(org.gudu0.AwareMemory.entities.SplitterEntity.Variant.LR);
        else if (lOk) s.setVariant(org.gudu0.AwareMemory.entities.SplitterEntity.Variant.FL);
        else if (rOk) s.setVariant(org.gudu0.AwareMemory.entities.SplitterEntity.Variant.FR);
    ...
    public void rebuildEntityAt(int cx, int cy) {
        ...
        TileEntity created = switch (id) {
            case WorldGrid.TILE_CONVEYOR -> new ConveyorEntity(cx, cy, rot);
            case WorldGrid.TILE_SELLPAD  -> new SellpadEntity(cx, cy, rot);
            case WorldGrid.TILE_SPAWNER  -> new SpawnerEntity(cx, cy, rot);
            case WorldGrid.TILE_CRUSHER -> new CrusherEntity(cx, cy, rot);
            case WorldGrid.TILE_SMELTER -> new SmelterEntity(cx, cy, rot);
            case WorldGrid.TILE_SPLITTER_FL -> new SplitterEntity(cx, cy, rot, SplitterEntity.Variant.FL);
            case WorldGrid.TILE_SPLITTER_FR -> new SplitterEntity(cx, cy, rot, SplitterEntity.Variant.FR);
            case WorldGrid.TILE_SPLITTER_LR -> new SplitterEntity(cx, cy, rot, SplitterEntity.Variant.LR);
            case WorldGrid.TILE_MERGER -> new MergerEntity(cx, cy, rot);

            // TODO: add others here as you implement them
            default -> null;
        };
    ...
    private void refreshConveyorShapeAt(int cx, int cy) {
        TileEntity te = getEntity(cx, cy);
        if (!(te instanceof ConveyorEntity c)) return;
        ...
        var cur = c.getShape();
        boolean curOk = switch (cur) {
            case STRAIGHT -> straightIn;
            case TURN_LEFT -> leftIn;
            case TURN_RIGHT -> rightIn;
        };
        if (curOk) return;
    ...
    public record ItemRenderInfo(Item item, float x, float y) {}
}
